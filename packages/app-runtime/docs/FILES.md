...
| [`src/module-contract.ts`](../src/module-contract.ts) | SoT du contrat de module marque (P2.c/H9 + volet 2 F3.4) : `BrandModuleDef` (`assistantSources`, `onboarding`) + `createBrandModuleRegistry` (collecteurs génériques dont `collectAssistantSources` / `collectOnboardingContent`) |
