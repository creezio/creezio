...
| [`src/tunnel-cf-client.ts`](../src/tunnel-cf-client.ts) | Client API Cloudflare v4 (Node pur) : `ensureCfTunnel` (auto-provision instance au boot), `ensureCfAgentTunnel`/`ensureCfAgentTunnelDns`/`removeCfTunnelAgentRule` (T7 — tunnel dédié agent + migration douce), DNS/e-mail, deprovision. |
| [`src/tunnel-cf.ts`](../src/tunnel-cf.ts) | Helpers purs tunnel (zéro réseau) : slugs réservés, règles ingress (`buildTunnelIngressRules`, `buildAgentTunnelIngressRules` T7), specs DNS, hostnames agent, URLs publiques. |
