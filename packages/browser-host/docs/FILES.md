...
|---------|------|--------------|
| `chromium-process.ts` | Spawn/supervision du binaire Chromium : args (`--remote-debugging-port=0`, `--user-data-dir` créé en `0700`, `--proxy-server` optionnel, sandbox, headless/Xvfb), détection binaire (`CREEZIO_CHROMIUM_BIN` prioritaire), purge des verrous `Singleton*` obsolètes, extraction de l'URL websocket CDP. | `launchChromium`, `findChromiumBinary`, `ChromiumProcess` |
...
| `shared-driver.ts` | Exécution des verbes `external_*` au-dessus d'un transport CDP abstrait (`CdpTransport`) — partagé Electron/serveur. | `runDriverVerb`, `driverVerbOf`, `CdpTransport` |
