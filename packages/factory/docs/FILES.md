...
| [`src/server-docker-cli.ts`](../src/server-docker-cli.ts) | CLI `creezio server-docker` — create/start/stop/rm/logs/ls, build/up/down/proof, `publish` (+ rétention, `--release` = déclaration draft dans l'app admin), `registry-gc` (GC registre local), `admin up`, `agent up`, `enroll`, `ensure-owner` (seed + persist `secrets.env` 600). `update` préserve un sidecar cloudflared / refuse si hostname public sans tunnel ; `migrate-stack` seul retire le sidecar (même tunnel). |
...
| [`src/server-docker-registry-gc.ts`](../src/server-docker-registry-gc.ts) | `creezio server-docker registry-gc` — GC fail-closed du registre Docker local (`registry:2`) : API v2 catalogue/tags/delete, `--keep N` (défaut 2), tags des conteneurs en cours toujours conservés, `registry garbage-collect`, `--dry-run`. |
