...
for dir in "${SRC_DIRS[@]}"; do
  ...
  if [[ "${found_meili}" -eq 0 ]]; then
    if copy_one "${dir}/meilisearch-win.exe" "${STAGE}/meilisearch-win.exe"; then
      found_meili=1
      # Alias runtime kit (compose-brand-os cherche meili.exe sous Windows).
      cp -a "${STAGE}/meilisearch-win.exe" "${STAGE}/meili.exe"
      echo "▸ ${STAGE}/meili.exe ← alias meilisearch-win.exe"
    elif copy_one "${dir}/meili.exe" "${STAGE}/meili.exe"; then
      found_meili=1
      cp -a "${STAGE}/meili.exe" "${STAGE}/meilisearch-win.exe"
      echo "▸ ${STAGE}/meilisearch-win.exe ← alias meili.exe"
    fi
  fi
...
if [[ "${found_cf}" -eq 0 || "${found_meili}" -eq 0 ]]; then
  ...
  echo "  meilisearch-win.exe|meili.exe: ${found_meili}" >&2
