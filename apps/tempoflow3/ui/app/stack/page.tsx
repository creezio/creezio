/**
 * Page métier Mes produits — générée --from-prd.
 * Liste réelle via API brand (plus un stub vide).
 */
async function loadItems() {
  const base = process.env.METIER_BASE_URL || "http://127.0.0.1:18791";
  try {
    const res = await fetch(`${base}/api/v1/brand/stack_items`, { cache: "no-store" });
    if (!res.ok) return [];
...
export default async function Page() {
  ...
  return (
    <section>
      <h1>Mes produits</h1>
      <p>Entité <code>stack_items</code> — {items.length} élément(s).</p>
      <ul>
        {items.map((item) => (
