/**
 * Page métier Panier — générée --from-prd.
...
async function loadItems() {
  ...
  try {
    const res = await fetch(`${base}/api/v1/modules/panier_lignes`, { cache: "no-store" });
    if (!res.ok) return [];
...
export default async function Page() {
  ...
  return (
    <section>
      <h1>Panier</h1>
      <p>Entité <code>panier_lignes</code> — {items.length} élément(s).</p>
      <ul>
        {items.map((item) => (
      ...
      <p>UI interactive : <code>resources/renderer/index.html#panier</code></p>
    </section>
  );
}
