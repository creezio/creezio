/**
 * Page métier Marketplaces — générée --from-prd.
 * Liste réelle via API brand (plus un stub vide).
 */
async function loadItems() {
  const base = process.env.METIER_BASE_URL || "http://127.0.0.1:18791";
  try {
    const res = await fetch(`${base}/api/v1/brand/marketplaces`, { cache: "no-store" });
    if (!res.ok) return [];
...
export default async function Page() {
  ...
  return (
    <section>
      <h1>Marketplaces</h1>
      <p>Entité <code>marketplaces</code> — {items.length} élément(s).</p>
      <ul>
        {items.map((item) => (
