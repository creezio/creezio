/**
 * Page métier Prix — générée --from-prd.
...
async function loadItems() {
  ...
  try {
    const res = await fetch(`${base}/api/v1/modules/prix`, { cache: "no-store" });
    if (!res.ok) return [];
...
export default async function Page() {
  ...
  return (
    <section>
      <h1>Prix</h1>
      <p>Entité <code>prix</code> — {items.length} élément(s).</p>
      <ul>
        {items.map((item) => (
      ...
      <p>UI interactive : <code>resources/renderer/index.html#prix</code></p>
    </section>
  );
}
