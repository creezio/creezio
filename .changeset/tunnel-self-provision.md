...
- **CLI** : `create` forwarde le contrat CF vers `cf.env` (verify du token, aucun `/reserve`) ; `rm` déprovisionne via l'API CF directe (DNS + tunnel) ; `enroll` gère l'ingress `agent[-.]{slug}` via le client CF ; `migrate-stack` bascule sidecar/legacy → in-container.
