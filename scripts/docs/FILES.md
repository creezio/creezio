...
| [`test-phase-server-docker-registry-gc.mjs`](../test-phase-server-docker-registry-gc.mjs) | Gate T11 — `server-docker registry-gc` : plan/rétention/in-use + mock HTTP fail-closed ; live `registry:2` éphémère ou skip explicite si Docker absent. |
