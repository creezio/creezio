...
| [`test-phase-nav-catalog.mjs`](../test-phase-nav-catalog.mjs) | Gate NAV-1 — catalogue nav OS : merge pur, collision id/href, feature-off, seed registre, factory chrome sans `const OS_NAV` ni literal `/granola`, factory deps sans `@creezio/granola` / `@creezio/grokbot` / `@creezio/nav` (non publiés). |
