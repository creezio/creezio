...
test("nav-catalog : factory deps générées sans @creezio/granola ni @creezio/grokbot ni @creezio/nav", async () => {
  const src = fs.readFileSync(FACTORY_OS_UI, "utf8");
  assert.doesNotMatch(
    src,
    /["']@creezio\/granola["']\s*:/,
  ...
  assert.doesNotMatch(
    src,
    /["']@creezio\/nav["']\s*:/,
    "générateur UI : pas de dep npm @creezio/nav (non publié)",
  );
  assert.doesNotMatch(
    src,
  ...
  assert.doesNotMatch(
    src,
    /transpilePackages:[\s\S]*["']@creezio\/nav["']/,
    "générateur UI : pas de transpilePackages @creezio/nav",
  );
  ...
  for (const listName of ["SERVER_CREEZIO_DEPS", "CLIENT_CREEZIO_DEPS"]) {
    ...
    assert.ok(!names.includes("nav"), `${listName} : pas de nav`);
  }
