...
async function bootTf3(envExtra = {}) {
...
test("plugins feature-off par défaut + endpoint", async () => {
  ...
  const build = spawnSync(
    ...
    [path.join(ROOT, "node_modules/typescript/bin/tsc"), "-p", "tsconfig.electron.json"],
    {
      encoding: "utf8",
      cwd: TF3,
      env: { ...process.env, CREEZIO_ROOT: ROOT, NODE_PATH: path.join(ROOT, "node_modules") },
    },
  );
  ...
  const handle = await bootTf3();
  try {
    const status = await (await fetch(`${handle.baseUrl}/api/v1/os/status`)).json();
    assert.equal(status.hosts.plugins, "feature-off");
    const plugins = await (await fetch(`${handle.baseUrl}/api/v1/os/plugins`)).json();
