...
- Gate : `test-phase-os-ui-scaffold` + `test-phase-nav-catalog` — le
  chrome / `package.json` généré ne contient pas `"@creezio/granola"`,
  `"@creezio/grokbot"` ni `"@creezio/nav"` comme dependency (les hrefs
  `/granola` via le catalogue restent OK ; le loader chrome s'importe
  depuis `@creezio/shell-ui/ui`).
