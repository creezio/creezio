...
| Lockfile Docker marque neuve | `npm ci` de l'image exige `server/package-lock.json` aligné sur `package.json`. **Ne jamais** « corriger » en lançant `npm install` dans `server/` puis en remuant `node_modules` (ça casse le symlink monorepo `server/node_modules` → `../node_modules` et boucle). Gestes : `creezio server-docker create|build` (régénère le lock via `ensureBrandPackageLocks`) ; le push factory (`maybePushBrandRepos`) génère les locks avant commit ; le Dockerfile a un fallback `npm ci \|\| npm install --omit=dev`. Gate : `test-phase-factory-lockfile`. |
